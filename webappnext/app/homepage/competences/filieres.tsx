...
export default function Filieres() {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8080/select/selectFilieres.php", form_data)
      .then((response) => {
  ...
  function filiereClicked(str: string) {
    let res: any = nomFilieres.find(
      (element: string): boolean => str == element[0]
    );
    if (res != undefined) {
  ...
  return (
    <>
      <div
        id="cards-container"
        className={"w-full px-3 flex flex-wrap justify-center gap-10 gap-y-8"}
      >
