...
select `learnagement`.`INFO_module`.`code` AS `code`,
       ...
       from (((((`learnagement`.`INFO_module` 
            ...
            join `learnagement`.`INFO_vue_parameters` on(`learnagement`.`INFO_module`.`code` = convert(`learnagement`.`INFO_vue_parameters`.`module` using utf8))) 
       ...
       group by `learnagement`.`INFO_module`.`code`,
...
select `learnagement`.`INFO_module`.`id_module` AS `id_module`,
       `learnagement`.`INFO_module`.`code` AS `code`,
...
select concat(`learnagement`.`INFO_enseignant`.`prenom`,' ',`learnagement`.`INFO_enseignant`.`nom`) AS `responsable`,
       ...
       group_concat(distinct `learnagement`.`INFO_module`.`code` separator ', ') AS `modules`
